...
from core.conventions import TENOR_DAYS, FWD_YIELD_TENORS, DELTA_POINTS, calendar_for_pair
...
def get_sabr_params(vol_grid: pd.Series, F: float, T: float,
                    r_f: float) -> Optional[Tuple[float, float]]:
    """
    The calibrated (nu, rho) for one pillar's quoted smile, or None if the fit
    did not converge / fewer than 3 pillars were available.

    Exactly the same calibration get_sabr_vol_at_K performs — this returns the
    SHAPE parameters instead of evaluating the curve at a strike. nu is the
    vol-of-vol, rho the spot/vol correlation, both belonging to the
    arbitrage-free surface the book is actually marked on.

    WHY THIS EXISTS. The alternative is the closed form off BF25/RR25 in
    dataset.get_smile_nu_rho. Measured on 2,583 USDJPY (date, tenor) fits, that
    closed form TRACKS this nu almost perfectly — correlation 0.92 to 0.99 —
    but overshoots it by a near-constant 1.28x (sd 0.04), and its rho has the
    wrong term structure outright: the calibrated rho decays towards zero and
    changes sign by 1Y, while 2.5*RR25/sigma stays negative. Since the fit
    already happens on the pricing path, reading the parameters off it is
    strictly more accurate at no extra cost.

    NOTE ON beta. SABR's nu is the vol-of-vol of alpha = sigma * F^(1-beta),
    not of sigma_ATM, because  d log alpha = d log sigma + (1-beta) d log F.
    At beta=0.5 and G10 spot vols that wedge is about 1.4% — immaterial in
    practice, but the breakeven buckets treat nu as sigma's own relative vol,
    so the two are not literally the same object and it is worth knowing.
    """
    sabr, _, _, _ = _fit_sabr_to_grid(vol_grid, F, T, r_f)
    if sabr is None:
        return None
    nu, rho = float(sabr.nu()), float(sabr.rho())
    if not (np.isfinite(nu) and np.isfinite(rho)) or nu <= 0.0:
        return None
    return nu, rho
