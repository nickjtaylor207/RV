...
def BF10D_VolAdjusted_WindowLookback(currency_list, tenors, lookback_days=252):
    def fetch_BF_and_ATM_data(currency_list: List[str], tenor_list: List[str]):
        ...
        df_bf_by_tenor: Dict[str, pd.DataFrame] = {}
        df_atm_by_tenor: Dict[str, pd.DataFrame] = {}
        df_bf_adj_by_tenor: Dict[str, pd.DataFrame] = {}  
        for tenor in tenor_list:
            bf_data_dict: Dict[str, pd.Series] = {}
            atm_data_dict: Dict[str, pd.Series] = {}
            for ccy_pair in base_ccy:
                ticker_BF = f"{ccy_pair}10B{tenor} BGN Curncy"
                ...
                try:
                    data_BF = blp.bdh(
                        ...
                        end_date=end_date)
                    data_BF.columns = data_BF.columns.get_level_values(1)
                    bf_data_dict[ccy_pair] = data_BF['PX_LAST']
                    data_ATM = blp.bdh(
                        tickers=ticker_ATM,
                        flds="PX_LAST",
                        start_date=start_date,
                    ...
                    continue
            if not bf_data_dict or not atm_data_dict:
                continue
            df_bf = pd.DataFrame(bf_data_dict).dropna()
            df_atm = pd.DataFrame(atm_data_dict).dropna()
            if df_bf.empty or df_atm.empty:
                continue
            df_aligned = pd.concat([df_bf, df_atm], axis=1, keys=['BF', 'ATM']).dropna()
            if df_aligned.empty:
                continue
            df_bf = df_aligned['BF']
            df_atm = df_aligned['ATM']
            df_bf_adj = df_bf.div(df_atm) * 100  
            df_bf_by_tenor[tenor] = df_bf
            df_atm_by_tenor[tenor] = df_atm
            df_bf_adj_by_tenor[tenor] = df_bf_adj
            latest_date = df_bf_adj.index[-1]
            currency_pairs = list(permutations(base_ccy, 2))
            pair_results = []
            for ccy_pair1, ccy_pair2 in currency_pairs:
                if ccy_pair1 not in df_bf_adj.columns or ccy_pair2 not in df_bf_adj.columns:
                    continue
                current_spread = df_bf_adj.loc[latest_date, ccy_pair1] - df_bf_adj.loc[latest_date, ccy_pair2]
                historical_spreads = df_bf_adj[ccy_pair1] - df_bf_adj[ccy_pair2]
                ...
                current_bf1 = df_bf.loc[latest_date, ccy_pair1]
                current_bf2 = df_bf.loc[latest_date, ccy_pair2]
                ...
                pair_result = {
                    ...
                    'Current_BF1': round(current_bf1, 4),
                    'Current_BF2': round(current_bf2, 4),
                    'Current_ATM1': round(current_atm1, 4),
                    'Current_ATM2': round(current_atm2, 4),
                    'Current_BF1_VolAdj': round(current_bf1 / current_atm1 * 100, 4),
                    'Current_BF2_VolAdj': round(current_bf2 / current_atm2 * 100, 4),
        ...
        return results_by_tenor, df_bf_by_tenor, df_atm_by_tenor, df_bf_adj_by_tenor
    ...
    results_by_tenor, df_bf_by_tenor, df_atm_by_tenor, df_bf_adj_by_tenor = fetch_BF_and_ATM_data(
    ...
    return {
        ...
        'df_bf_raw': df_bf_by_tenor,
        'df_atm': df_atm_by_tenor,
        'df_bf_vol_adjusted': df_bf_adj_by_tenor}
    


def BF10D_VolAdjusted_1YWindow(currency_list, tenors):
    def fetch_BF_and_ATM_data(currency_list: List[str], tenor_list: List[str]):
        end_date = datetime.now().strftime("%Y-%m-%d")
        start_date = (datetime.now() - timedelta(days=390)).strftime("%Y-%m-%d")  # Only need 2 years max
        base_ccy = sorted(set(currency_list))
        results_by_tenor: Dict[str, pd.DataFrame] = {}
        df_bf_by_tenor: Dict[str, pd.DataFrame] = {}
        df_atm_by_tenor: Dict[str, pd.DataFrame] = {}
        ...
        for tenor in tenor_list:
            ...
            for ccy_pair1, ccy_pair2 in currency_pairs:
                ...
                one_year_ago = latest_date - timedelta(days=365)
                past_year_spreads = historical_spreads.loc[historical_spreads.index >= one_year_ago]
                if len(past_year_spreads) < 2:
                    continue
                percentile_1_year = percentileofscore(past_year_spreads, current_spread)
                spread_volatility_1y = past_year_spreads.std()
                mean_spread_1y = past_year_spreads.mean()
                relative_position_1y = abs(current_spread - mean_spread_1y) / spread_volatility_1y if spread_volatility_1y > 0 else 0
                current_bf1 = df_bf.loc[latest_date, ccy_pair1]
                current_bf2 = df_bf.loc[latest_date, ccy_pair2]
                current_atm1 = df_atm.loc[latest_date, ccy_pair1]
                current_atm2 = df_atm.loc[latest_date, ccy_pair2]
                pair_result = {
                    ...
                    'Current_BF1_VolAdj': round(current_bf1 / current_atm1 * 100, 4),
                    'Current_BF2_VolAdj': round(current_bf2 / current_atm2 * 100, 4),
                    'Spread_Mean_1Y': round(mean_spread_1y, 4),
                    'Spread_STD_1Y': round(spread_volatility_1y, 4),
                    'Spread_ZScore_1Y': round(relative_position_1y, 4),
                    'Percentile_1Y': round(percentile_1_year, 3)}
                pair_results.append(pair_result)
            results_by_tenor[tenor] = pd.DataFrame(pair_results)
        return results_by_tenor, df_bf_by_tenor, df_atm_by_tenor, df_bf_adj_by_tenor
    def find_ave_allCCY_equalWeights_1Y(results_by_tenor, tenors):
        tenor_results = {}
        for tenor in tenors:
            if tenor in results_by_tenor:
                df = results_by_tenor[tenor]
    ...
    results_by_tenor, df_bf_by_tenor, df_atm_by_tenor, df_bf_adj_by_tenor = fetch_BF_and_ATM_data(
        currency_list, tenors)
    ave_tenorBuckets_equalWeights_1Y = find_ave_allCCY_equalWeights_1Y(results_by_tenor, tenors)
    return {
        'results_by_tenor': results_by_tenor,
        'ave_results_1Y_equalWeights': ave_tenorBuckets_equalWeights_1Y,
        'df_bf_raw': df_bf_by_tenor,
        'df_atm': df_atm_by_tenor,
        'df_bf_vol_adjusted': df_bf_adj_by_tenor}
...
ccys_em = ['USDBRL', 'USDMXN', 'USDZAR', 'USDTRY', 'USDRUB', 'USDCLP', 'USDCOP', 'USDPLN', 'USDHUF']
...
tenors = ['1M']
results = BF10D_VolAdjusted_WindowLookback(CCYS, tenors)


print(results['ave_results_by_tenor_equalWeights']['1M'])
