...
def Data_VolAdjusted_RR_Multiple(currency_pairs, tenor, days):
    ...
    tenor_code = tenor_map[tenor]
    df_all = {}
    for ccy_pair in currency_pairs:
        rr_ticker = f"{ccy_pair}25R{tenor_code} BGN Curncy"
        atm_ticker = f"{ccy_pair}V{tenor_code} Curncy"
        try:
            rr_data = blp.bdh(
                tickers=rr_ticker,
                flds=["PX_LAST"],
            ...
            atm_data = blp.bdh(
                ...
                Per="D")
            rr_data.columns = ['RR']
            atm_data.columns = ['ATM']
            combined = pd.concat([rr_data, atm_data], axis=1).dropna()
            combined[ccy_pair] = combined['RR'] / combined['ATM']
            df_all[ccy_pair] = combined[[ccy_pair]]
        except Exception as e:
            print(f"Error fetching data for {ccy_pair} {tenor}: {e}")
            continue
    if not df_all:
        raise ValueError("No data could be fetched for any currency pair")
    df_result = pd.concat(df_all, axis=1)
    df_result.columns = df_result.columns.droplevel(0) if isinstance(df_result.columns, pd.MultiIndex) else df_result.columns
...
class Screen_SinglePair_Metrics:
    def __init__(self, currency_pairs, tenor, days):
        self.ticker1 = currency_pairs[0]
        self.ticker2 = currency_pairs[1]
        self.tenor = tenor
        self.df = Data_VolAdjusted_RR_Multiple(currency_pairs, tenor, days).dropna()
        self.log_df = np.log(np.abs(self.df) + 1e-10)  # Add small constant to handle potential zeros/negatives
        self.spread = None
        self.alpha = None
        self.beta = None
    ...
    def hurst_exponent(self, lags_range=(2, 100)):
        ...
        logp1 = np.log(np.abs(px1.values) + 1e-10)
        logp2 = np.log(np.abs(px2.values) + 1e-10)
...
class Screen_MultiPair_Metrics:
    def __init__(self, currency_pairs, tenor, days):
        ...
        self.tenor = tenor
        self.days = days
        self.pairs = list(combinations(currency_pairs, 2))
        self.screeners = {}
        for pair in self.pairs:
            pair_key = f"{pair[0]} / {pair[1]}"
            self.screeners[pair_key] = Screen_SinglePair_Metrics(list(pair), tenor, days)
...
def analyze_rr_multiple_tenors_to_excel(currency_pairs, tenors, days=730, 
                                        output_file='rr_cointegration_analysis.xlsx'):
    all_results = {}
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        for tenor in tenors:
            print(f"\nAnalyzing tenor: {tenor}")
            try:
                analyzer = Screen_MultiPair_Metrics(currency_pairs, tenor, days)
...
def create_summary_sheet(results_dict, output_file='rr_cointegration_analysis.xlsx'):
...
def run_rr_multi_tenor_analysis(currency_pairs=None, tenors=None, days=730,
                                 output_file='rr_cointegration_results.xlsx'):
    ...
    print("=" * 70)
    print("VOL-ADJUSTED 25D RISK REVERSAL COINTEGRATION ANALYSIS")
    print("=" * 70)
    ...
    print("=" * 70)
    results = analyze_rr_multiple_tenors_to_excel(
...
if __name__ == "__main__":
    ...
    days = 365 * 1
    output_file = 'fx_rr_cointegration_analysis_1Y.xlsx'
    
    results = run_rr_multi_tenor_analysis(currencies, tenors, days, output_file)
